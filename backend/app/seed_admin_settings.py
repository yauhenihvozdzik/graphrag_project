...
# Off-topic defaults — used as seed values; runtime overrides come from DB
_OFF_TOPIC_KEYWORDS = [
...
_OFF_TOPIC_BLACKLIST = [
    "погода", "прогноз погоды", "спорт", "футбол", "хоккей", "баскетбол",
    "музык", "песн", "фильм", "кино", "сериал", "актёр", "актер",
    "рецепт", "готовк", "еда", "кулинар", "ресторан",
    "игра", "видеоигр", "steam", "playstation", "xbox",
    "знаменитост", "инстаграм", "tiktok", "ютуб",
    "мем", "анекдот", "шутк", "прикол",
    "котик", "собак", "животн",
    "сколько времени", "который час", "какая дата",
]
...
DEFAULTS: list[tuple[str, str, str, str]] = [
    ...
    # ── off_topic ──
    ("off_topic", "keywords", json.dumps(_OFF_TOPIC_KEYWORDS, ensure_ascii=False), "Whitelist: ключевые слова бизнес-домена (усиление релевантности)"),
    ("off_topic", "blacklist", json.dumps(_OFF_TOPIC_BLACKLIST, ensure_ascii=False), "Blacklist: явно нерелевантные темы для блокировки"),
    # ── stop_tokens ──
