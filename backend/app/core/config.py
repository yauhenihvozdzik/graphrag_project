...
class Settings:
    def __init__(self):
        ...
        self.PROJECT_NAME = os.getenv("PROJECT_NAME", "GraphRAG Platform")
        self.VERSION = os.getenv("VERSION", "1.0.0")
        self.DESCRIPTION = os.getenv("DESCRIPTION", "Защищённая платформа GraphRAG для корпоративных знаний")
        self.API_V1_STR = os.getenv("API_V1_STR", "/api/v1")
        self.DEBUG = os.getenv("DEBUG", "false").lower() in ("true","1","yes")
        self.ALLOWED_ORIGINS = parse_list_from_env("ALLOWED_ORIGINS", ["http://localhost:3000"])
        self.JWT_SECRET_KEY = os.getenv("JWT_SECRET_KEY", "")
        self.JWT_PUBLIC_KEY_PATH: str = os.getenv("JWT_PUBLIC_KEY_PATH", "./keys/public.pem")
        self.JWT_PRIVATE_KEY_PATH: str = os.getenv("JWT_PRIVATE_KEY_PATH", "./keys/private.pem")
        ...
        self.NEO4J_MAX_CONNECTION_POOL_SIZE = int(os.getenv("NEO4J_MAX_CONNECTION_POOL_SIZE","50"))
        self.QDRANT_HOST = os.getenv("QDRANT_HOST","localhost")
        self.QDRANT_PORT = int(os.getenv("QDRANT_PORT","6333"))
        self.QDRANT_GRPC_PORT = int(os.getenv("QDRANT_GRPC_PORT","6334"))
        self.QDRANT_API_KEY = os.getenv("QDRANT_API_KEY","")
        self.QDRANT_COLLECTION = os.getenv("QDRANT_COLLECTION","graphrag_documents")
        self.QDRANT_VECTOR_SIZE = int(os.getenv("QDRANT_VECTOR_SIZE","1024"))
        self.POSTGRES_HOST = os.getenv("POSTGRES_HOST","localhost")
        self.POSTGRES_PORT = int(os.getenv("POSTGRES_PORT","5432"))
        self.POSTGRES_DB = os.getenv("POSTGRES_DB","graphrag_db")
        ...
        self.OTEL_ENABLED = os.getenv("OTEL_ENABLED","true").lower() in ("true","1","yes")
        self.OTEL_EXPORTER_ENDPOINT = os.getenv("OTEL_EXPORTER_ENDPOINT","http://localhost:4317")
        self.OTEL_SERVICE_NAME = os.getenv("OTEL_SERVICE_NAME","graphrag-backend")
        self.GUARDRAILS_ENABLED = os.getenv("GUARDRAILS_ENABLED","true").lower() in ("true","1","yes")
        ...
        self.ENTITY_EXTRACTION_BATCH_SIZE = int(os.getenv("ENTITY_EXTRACTION_BATCH_SIZE","5"))
        self.GRAPH_COMMUNITY_ALGORITHM = os.getenv("GRAPH_COMMUNITY_ALGORITHM","louvain")
        ...
        self.SMTP_PORT = os.getenv("SMTP_PORT","1025")
        ...
        self.S3_BUCKET = os.getenv("S3_BUCKET","graphrag-documents")
    ...
    def _validate_jwt_secret(self):
        if not self.JWT_SECRET_KEY:
            ...
            logger.error("JWT_SECRET_KEY must be set in environment")
            raise ValueError("JWT_SECRET_KEY must be set in environment")

    @property
    def DATABASE_URL(self) -> str:
        """Async DSN for the application (asyncpg)."""
        return os.getenv("DATABASE_URL") or f"postgresql+asyncpg://{self.POSTGRES_USER}:{self.POSTGRES_PASSWORD}@{self.POSTGRES_HOST}:{self.POSTGRES_PORT}/{self.POSTGRES_DB}"

    @property
    def SYNC_DATABASE_URL(self) -> str:
        """Sync DSN for Alembic / legacy tools."""
        return os.getenv("SYNC_DATABASE_URL") or f"postgresql://{self.POSTGRES_USER}:{self.POSTGRES_PASSWORD}@{self.POSTGRES_HOST}:{self.POSTGRES_PORT}/{self.POSTGRES_DB}"

    @property
    def postgres_dsn(self) -> str: return self.SYNC_DATABASE_URL

    @property
    def postgres_async_dsn(self) -> str: return self.DATABASE_URL
