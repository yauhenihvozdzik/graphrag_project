...
class SettingsRegistry:
    ...
    _OFF_TOPIC_KEYWORDS_FALLBACK: list[str] = [
        "закон", "статья", "кодекс", "документ", "договор", "отчёт", "отчет",
        "инструкци", "регламент", "приказ", "бизнес", "систем", "настройк",
        "пользовател", "модул", "сотрудник", "отдел", "рол", "ERP", "CRM",
    ]

    _OFF_TOPIC_BLACKLIST_FALLBACK: list[str] = [
        "погода", "спорт", "футбол", "музык", "фильм", "кино", "сериал",
        "рецепт", "еда", "игра", "видеоигр", "мем", "анекдот", "шутк",
    ]
    ...
    def get_off_topic_keywords(self) -> list[str]:
        ...
        return self._OFF_TOPIC_KEYWORDS_FALLBACK

    def get_off_topic_blacklist(self) -> list[str]:
        val = self.get("off_topic", "blacklist")
        if val:
            return val if isinstance(val, list) else [val]
        return self._OFF_TOPIC_BLACKLIST_FALLBACK
